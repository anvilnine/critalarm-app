...
/// `schedule` throws [ReminderScheduleRefused] so a refused reminder is
/// never recorded as sent.
final class NativeReminderScheduler implements ReminderScheduler {
  NativeReminderScheduler([MethodChannel? channel])
  ...
  final _taps = StreamController<ReminderTap>.broadcast();
  ...
  static Map<String, Object?> encode(ReminderRequest request) => {
    'id': request.id,
  ...
  static ReminderTap? decodeTap(Object? raw) {
    if (raw is! Map) return null;
    final wire = raw['kind'];
    final kind = wire is String ? ReminderKind.fromWire(wire) : null;
    final action = raw['action'];
    if (kind == null || action is! String?) return null;
    return ReminderTap(
      kind: kind,
      actionId: action ?? ReminderActionIds.open,
  ...
  static PendingReminder? decodePending(Object? raw) {
    ...
    return PendingReminder(
      id: id,
      kind: kind is String ? ReminderKind.fromWire(kind) : null,
      fireAt: hasTime
  ...
  Future<void> schedule(ReminderRequest request) async {
    ...
    try {
      ...
      throw const ReminderScheduleRefused();
    } on PlatformException {
      throw const ReminderScheduleRefused();
    }
    // Both platforms answer true once the reminder is armed.
    if (answer != true) throw const ReminderScheduleRefused();
  }
  ...
  bool _isRepeat(ReminderTap tap) {
    final id = tap.tapId;
