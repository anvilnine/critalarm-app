...
class ReminderSettingsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) {
        final cubit = getIt<ReminderSettingsCubit>();
        unawaited(cubit.load());
        return cubit;
      },
      child: const _ReminderSettingsContent(),
    );
  }
}

class _ReminderSettingsContent extends StatefulWidget {
  const _ReminderSettingsContent();

  @override
  State<_ReminderSettingsContent> createState() =>
      _ReminderSettingsContentState();
}

class _ReminderSettingsContentState extends State<_ReminderSettingsContent>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed && mounted) {
      unawaited(context.read<ReminderSettingsCubit>().load());
    }
  }

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<ReminderSettingsCubit, ReminderSettingsState>(
