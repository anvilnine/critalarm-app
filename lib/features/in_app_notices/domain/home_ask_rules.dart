...
/// `ProAskRules`, it holds no widgets and reads the clock through the
/// `now` it is given.
///
...
class HomeAskRules {
  ...
  Future<HomeAsk> next() async {
    await _settle?.call();
    await noticeRepository.markFirstSeen();
    ...
    return decide(
      ...
      now: _now(),
      firstSeenAt: noticeRepository.getFirstSeenAt(),
      consentAskedAt: noticeRepository.getConsentAskedAt(),
      isConsentGiven:
          privacy != null &&
          privacy.analyticsEnabled &&
          privacy.crashReportingEnabled,
      reviewAskedAt: noticeRepository.getReviewAskedAt(),
      reviewAskCount: noticeRepository.getReviewAskCount(),
      lastAcknowledgedAt: newerOf(
        _newestAckedAt?.call(),
        noticeRepository.getLastAcknowledgedAt(),
      ),
      proAskedAt: noticeRepository.getProAskedAt(),
      isRinging: false,
      isWeb: _isWeb,
