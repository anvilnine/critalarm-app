...
class SharedPrefsLocalReminderStore implements LocalReminderStore {
  const SharedPrefsLocalReminderStore(this._prefs);

  final SharedPreferences _prefs;

  static const localRemindersKey = 'reminder_switch_reminders';
  ...
  static const planHeadsUpsKey = 'reminder_plan_notices_sent';
  ...
  @override
  LocalReminderSwitches readSwitches() => LocalReminderSwitches(
    reminders:
        _prefs.getBool(localRemindersKey) ??
        LocalReminderSwitches.defaults.reminders,
    offers: _prefs.getBool(offersKey) ?? LocalReminderSwitches.defaults.offers,
  );

  @override
  Future<void> writeSwitches(LocalReminderSwitches switches) async {
    await _prefs.setBool(localRemindersKey, switches.reminders);
  ...
  List<PlannedRecord> readPlanned() {
    ...
    try {
      ...
      return [
        for (final item in decoded) ?PlannedRecord.tryParse(item),
      ];
    } on FormatException {
      return const [];
    }
  ...
  @override
  Set<String> readPlanHeadsUpsSent() => _readSet(planHeadsUpsKey);

  @override
  Future<void> addPlanHeadsUpSent(String key) =>
      _addToSet(planHeadsUpsKey, key);
