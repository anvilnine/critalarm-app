...
abstract final class LocalReminderActionIds {
...
@immutable
final class LocalReminderAction {
...
@immutable
final class LocalReminderRequest {
  ...
  final List<LocalReminderAction> actions;
...
@immutable
final class LocalReminderSystemState {
  const LocalReminderSystemState({required this.notificationsAllowed});
...
final class LocalReminderTap {
  ...
  /// A `LocalReminderActionIds` value.
...
final class LocalReminderScheduleRefused implements Exception {
...
abstract interface class LocalReminderScheduler {
  ...
  Future<LocalReminderSystemState> systemState();

  Future<DeviceTimeZone> deviceTimeZone();

  /// Taps while the app runs.
  Stream<LocalReminderTap> get taps;

  /// A tap the platform is still holding, taken once.
  Future<LocalReminderTap?> takePendingTap();
}
