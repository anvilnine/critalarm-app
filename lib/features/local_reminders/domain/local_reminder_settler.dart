...
final class LocalReminderSettler {
  ...
  Future<void> _settle(PlannedRecord record) async {
    ...
    switch (record.kind) {
      case LocalReminderKind.fireDrill:
        final pool = record.poolIndex;
        if (pool != null) await _store.writeDrillLastIndex(pool);
      case LocalReminderKind.silentTopic:
        if (key != null) await _store.addSilentDone(key);
      case LocalReminderKind.backup:
        await _notices.dismissAccountNotice();
      case LocalReminderKind.planHeadsUp:
        if (key != null) await _store.addPlanHeadsUpSent(key);
      case LocalReminderKind.morningAfter:
        if (key != null) await _store.addMorningAfterDone(key);
      case LocalReminderKind.proLater:
        await _notices.clearProAskLater();
      case LocalReminderKind.reviewAsk:
      case LocalReminderKind.feedbackAsk:
