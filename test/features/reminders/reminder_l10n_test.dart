...
void main() {
  test('every key the copy reads resolves to English', () {
    final keys = [
      ...ReminderCopy.drillTitles,
      ...ReminderCopy.drillBodies,
      ...ReminderCopy.reviewTitles,
      ...ReminderCopy.reviewBodies,
      ...ReminderCopy.feedbackTitles,
      ...ReminderCopy.feedbackBodies,
    ...
    for (final key in keys) {
      final isPlural =
          ReminderCopy.pluralKeys.contains(key) ||
  ...
  test('every kind builds with no key or placeholder left over', () {
    for (final isIos in [true, false]) {
      final copy = ReminderCopy(isIos: isIos);
      for (final kind in ReminderKind.values) {
        final request = copy.build(
          ReminderLabSamples.candidate(kind, DateTime(2026, 10, 3, 10)),
        );
        final texts = [
          request.title,
          request.body,
