...
class _CountingNotices extends FakeInAppNoticeRepository {
  int expiryWrites = 0;
  @override
...
void main() {
  ...
  test('free to paid clears a pending ended sheet', () async {
    ...
    identity = account('acc_1', 'hosted');
    expect((await ending.read()).sheet, ProPlanSheet.none);
    expect(notices.proEndedDueFor, isNull);
  });

  test('a known expiry that passed asks the server again', () async {
    notices.proKnownExpiry = DateTime(2026, 9, 30);
    await ending.read();
    expect(refreshes, 1);
  });
