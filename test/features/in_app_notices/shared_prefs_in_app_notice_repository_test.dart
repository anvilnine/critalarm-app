...
void main() {
  test('Pro ending storage round-trips', () async {
    ...
    await repo.dismissProEndingNotice();
    expect(repo.getProEndingNoticeDismissedAt(), isNotNull);
  ...
  setUp(() async {
    SharedPreferences.setMockInitialValues({});
    repository = SharedPrefsInAppNoticeRepository(
      await SharedPreferences.getInstance(),
    );
  });
  ...
  test('"Remind me later" never counts as a "Not now"', () async {
    await repository.remindProAskLater();
    await repository.remindProAskLater();
    expect(repository.getProAskLaterAt(), isNotNull);
    expect(repository.getProAskDismissCount(), 0);
