...
void main() {
  ...
  Future<SharedPrefsLocalReminderStore> fresh([
    Map<String, Object> values = const {},
  ]) async {
    SharedPreferences.setMockInitialValues(values);
    return SharedPrefsLocalReminderStore(await SharedPreferences.getInstance());
  }

  setUp(() async => store = await fresh());

  test('starts with Reminders on and Offers off', () {
    expect(store.readSwitches(), LocalReminderSwitches.defaults);
  });

  test('keeps the switches', () async {
    await store.writeSwitches(LocalReminderSwitches.allOff);
    expect(store.readSwitches(), LocalReminderSwitches.allOff);
  });
  ...
  test('keeps planned records and skips ones it cannot read', () async {
    ...
    expect(read.id, 9100);
    expect(read.kind, LocalReminderKind.fireDrill);
  ...
  test('keeps the done sets and topics made here', () async {
    ...
    await store.addPlanHeadsUpSent('ends:2026-10-2');
    ...
    expect(store.readPlanHeadsUpsSent(), {'ends:2026-10-2'});
