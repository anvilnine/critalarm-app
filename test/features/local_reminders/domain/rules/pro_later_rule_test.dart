...
void main() {
  ...
  test('comes back at the first 10:00 thirty days later', () {
    final c = ProLaterRule.candidate(
      LocalReminderInputs(now: now, proLaterAt: DateTime(2026, 9, 1, 12)),
    )!;
    expect(c.id, LocalReminderIds.proLater);
    expect(c.fireAt, DateTime(2026, 10, 2, 10));
  });

  test('nothing without a "Remind me later"', () {
    expect(ProLaterRule.candidate(LocalReminderInputs(now: now)), isNull);
  });
