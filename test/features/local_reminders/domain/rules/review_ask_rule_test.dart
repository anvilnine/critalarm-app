...
void main() {
  ...
  LocalReminderInputs inputs({
    ...
    List<LocalReminderIncident> incidents = const [],
  ...
  test('plans 10:00 four days after the last ack', () {
    ...
    expect(c.id, LocalReminderIds.reviewAsk);
    expect(c.fireAt, DateTime(2026, 9, 24, 10));
    expect(c.args[LocalReminderArgs.pool], '0');
  });
  ...
  test('alternates the two lines with the ask count', () {
    ...
    expect(c.args[LocalReminderArgs.pool], '1');
  });
}
