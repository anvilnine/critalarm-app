...
void main() {
  ...
  const prod = LocalReminderTopic(name: 'prod-db', isCritical: true);
  ...
  test('plans Saturday 10:00 for a critical topic untested for 30+ days', () {
    ...
    expect(c.kind, LocalReminderKind.fireDrill);
    expect(c.id, LocalReminderIds.drill);
    expect(c.fireAt, DateTime(2026, 9, 26, 10));
    expect(c.args[LocalReminderArgs.topic], 'prod-db');
    expect(c.args[LocalReminderArgs.days], '37');
  ...
  test('skips normal topics and does nothing without a critical one', () {
    expect(
      FireDrillRule.candidate(
        LocalReminderInputs(
          now: now,
          topics: const [LocalReminderTopic(name: 'backups')],
  ...
  test('names the critical topic tested longest ago', () {
    final c = FireDrillRule.candidate(
      LocalReminderInputs(
        ...
        topics: const [
          prod,
          LocalReminderTopic(name: 'db-2', isCritical: true),
        ],
        lastTestAt: {
    ...
    expect(c.args[LocalReminderArgs.topic], 'db-2');
  });
  ...
  test('a topic never tested counts from when it was made', () {
    ...
    expect(c.args[LocalReminderArgs.days], '47');
  });
  ...
  test('after a real alarm the line never claims nothing rang', () {
    ...
    expect(c.args[LocalReminderArgs.pool], '${c.poolIndex}');
  });
