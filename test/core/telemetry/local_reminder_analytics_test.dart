...
void main() {
  ...
  late LocalReminderAnalytics analytics;

  setUp(() {
    gate = _RecordingGate();
    analytics = LocalReminderAnalytics(gate);
  });

  test('"Remind me later" has its own answer value', () async {
    await analytics.proAskAnswered(answer: LocalReminderAnalytics.remindLater);
  ...
  test('a reminder tap is logged with its kind and action', () async {
    final bindings = LocalReminderBindings(
